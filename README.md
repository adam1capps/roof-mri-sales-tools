...
interactive **McCallum HS, The App in Action** walkthrough. Six tabs, Campus → Blank Grids → MRI Report → Vent Map → Installed → The Bid, with
...
each party, an interactive **national footprint map** (city + state only, no addresses; green = installed/drying, orange = bid/scanned), and the
